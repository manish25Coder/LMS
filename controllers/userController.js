...
const cookieOptions={
    ...
    httpOnly:true,//because any one notable to do anythings from js
    secure:true
}
...
const register =async (req,res,next)=>{
    ...
    const user=await User.create({
        ...
        avatar:{
            public_id:email,
            secure_url:'https://res.cloudinary.com/freshpm/image/upload/c_scale,w_500/f_auto/q_auto/samples/bike.jpg'
        }
    });
    if(!user){
    ...
    user.password=undefined//user not able to see what password their are send

    const token=await user.generateJWTToken();

    res.cookie('token',token,cookieOptions)//donot have to login manually
...
const getProfile =async (req,res)=>{
    ...
    res.status(200).json({
        ...
        return next(new AppError('Failed to fetch profiles'))
    }
...
const forgotPassword=async(req,res,next)=>{
     ...
     try{
        await sendEmail(email,success,message);
        res.status(200).json({
            success:true,
